...
def file_view(request, pk, pk1):
	...
	if request.accepted_renderer.format == 'html':
		...
		if filnnwext[-1]=='jpg':
			...
			print(blobname,"blobname")
			pdfpath2="https://docullystorage.blob.core.windows.net/docullycontainer/"+blobname
			...
			print(blobname,"blobname")
			pdfpath="https://docullystorage.blob.core.windows.net/docullycontainer/"+blobname
		...
		for file in data['data']:
			if 'is_folder' in file:
				if file['is_folder']:
					...
					os.mkdir(''+directory+'')
					# break
					utils.get_sub_list(file['id'], directory)
					# break
				if not file['is_folder']:
					utils.download(str(file['path']), str(directory)+'/')
					# break
			else:
				if file['folder']['is_folder']:
					try:
						...
						# break
						utils.get_sub_list(file['folder']['id'], str(directory_name)+'/'+str(file['folder']['name']))
					except:
						print("File Exist")
					# break
				if not file['folder']['is_folder']:
					utils.download(str(file['folder']['path']), str(directory_name)+'/')
