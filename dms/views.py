		...
		for da in data:
			...
			try:
				...
				if member.is_la_user == True or member.is_dataroom_admin == True:
					...
					da['perm']['is_shortcut'] = perm_obj.is_access
			except:
				...
				if folder_obj == None:
					...
					if dataroom_folder_serializer.is_valid():
						...
						if str(dataroom_folder_serializer.data.get('name')).split('.')[1]=='docx' or str(dataroom_folder_serializer.data.get('name')).split('.')[1]=='doc' or str(dataroom_folder_serializer.data.get('name')).split('.')[1]=='odt':
							...
							cmd = 'libreoffice --headless --invisible --convert-to pdf '+pathsplit[-1]
							print(cmd,"cmd")
							print(os.system(cmd),"print cmd")
							os.system(cmd)
...
def file_view(request, pk, pk1):
	...
	if request.accepted_renderer.format == 'html':
		...
		context_data['url'] = base_url+'/file-view/'+str(pk)+'/'+str(pk1)+'/'
...
class bulkDownload(APIView):
	...
	def post(self, request, pk, format=None):
		...
		print(file_name,"file name before loop")
		print(os.mkdir('home/cdms_backend/cdms2/media/'+str(file_name)))
		print("folder")
		directory_name = 'home/cdms_backend/cdms2/media/'+''+str(file_name).replace(' ','\ ')+''
