    ...
    def get(self, request, pk, format=None):
        ...
        #             name = str(path_list[-1])
                    ...
                    #   raise Http404
                        ...
                        if ((extension=='.xlsx') or (extension==".xls")) and ((data_of_per2['is_excel_as_pdf']==True)):
                            import os, uuid, sys
                            from azure.storage.blob import BlockBlobService, PublicAccess
                            block_blob_service = BlockBlobService(account_name='docullystorage', account_key='ddIGey4fa6zz/FnWjMgPm5zN35BgIEDsaY6K18dpTFpkqUAJRD6efPBpXZGdBG8ICnyWWE8Y/PPGZQ0ajUeZTw==')
                            container_name ='docullycontainer'
...
class FileActivitybyDateReport(APIView):
    ...
    def get(self, request, pk, format=None):
        ...
        document = DataroomFolder.objects.filter(dataroom_id = pk, is_folder=False, created_date__gte=first_date, created_date__lte=todays_date, is_deleted=deleted)
        file_data = DataroomFolderSerializer(document, many=True).data
...
def file_view(request, pk, pk1):
    ...
    frontend_url = "https://services.docully.com"
    ...
    if request.accepted_renderer.format == 'html':
        ...
        if member.is_dataroom_admin == True or member.is_la_user==True:
            ...
            # print(view_serializer,"view_serializer")
        ...
        context_data['pathpptpdf']=context_data['path'].replace('.pptx','.pdf').replace('.docx','.pdf').replace('.ppt','.pdf').replace('.doc','.pdf')
        ...
        if member.is_dataroom_admin == True or member.is_la_user==True:
            ...
            if context_data['pdf'] ==True:
                ...
                <head> 
                    ...
                    <link href="https://services.docully.com/assets/css/innerviewr.css" rel="stylesheet" type="text/css" /> 
                                                                    </head> 
                                                                    ...
                                                                    </html>""".format('overflow: auto;','margin: 0px;padding: 0px; height: 100%; border: none;','display: block;width: 100%; border: none; overflow-y: auto; overflow-x: hidden;',context_data['path']) 

                f.write(message)
                f.close()
                # render(request,'/home/cdms_backend/cdms2/media/')
                context_data['path'] = 'https://services.docully.com/projectName/media/'+filename+".html"           
...
class DataroomActivity(APIView):
    ...
    def get(self, request, pk, format=None):
        ...
        print(datas,'_____________this api')
...
class IndexReportUserWise(APIView):
    ...
    def get(self, request, pk, pk1, format=None):
        ...
        writer.writerow(["This index report is of "+users.first_name+" "+users.last_name+" ("+users.email+") from _______Dataroom "+dataroom.dataroom_name])
        writer.writerow(header_data)
        new_list = []
        for value in datas:
        ...
        for new_value in new_list:
            ...
            if new_list[i][-1] is not None:
                new_list[i][-1] = dataroom.dataroom_name +'/'+ new_list[i][1] + '/' + new_list[i][-1]
...
def downloadfileskey(request, path, user):
    if request.method == "GET":
        ...
        member = DataroomMembers.objects.filter(member_id=user.id, dataroom_id=room_id_here).first()
        ...
        if (extension=='.pdf'):
            ...
            if ((extension=='.xlsx') or (extension==".xls")) and ((data_of_per2['is_excel_as_pdf']==True)):
                ...
                with open(pdf_filename, 'rb') as output:
                    ...
                    response['Content-Disposition'] = 'inline; filename=' + pdf_filename
                    return response
            elif ((extension=='.xlsx') or (extension==".xls")) and ((data_of_per2['is_doc_as_pdf']==True)):
                print("in xlsx")
                import os, uuid, sys
                from azure.storage.blob import BlockBlobService, PublicAccess
                block_blob_service = BlockBlobService(account_name='docullystorage', account_key='ddIGey4fa6zz/FnWjMgPm5zN35BgIEDsaY6K18dpTFpkqUAJRD6efPBpXZGdBG8ICnyWWE8Y/PPGZQ0ajUeZTw==')
        ...
        for file in data['data']:
            if 'is_folder' in file:
                if file['is_folder']:
                    ...
                    os.mkdir(''+directory+'')
                    # break
                    utils.get_sub_list(file['id'], directory,str(file['index']),'')
                    # break
                if not file['is_folder']:
                    utils.download(file['id'],str(file['path']), str(directory_name)+'/')
                    # break
            else:
                if file['folder']['is_folder']:
                    try:
                        ...
                        # break
                        utils.get_sub_list(file['folder']['id'], str(directory_name)+'/'+str(file['folder']['name']),str(file['index']),'','')
                    except:
                        print("File Exist")
                    # break
                if not file['folder']['is_folder']:
                    print('3',file['folder']['path'])
                    utils.download(file['id'],str(file['folder']['path']), str(directory_name)+'/',str(file['index']),'','')
